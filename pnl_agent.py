...
def parse_1c(path):
    ...
    leaves=[]; acct=None; item=None; item_val=None; item_kind=None; item_got=False; stack=[]
    def flush():
        # статья без разбивки по подразделению -> один лист с div=None
        if item is not None and item_val and not item_got:
    ...
    for r in range(1,ws.max_row+1):
        ...
        if o<=1:
            flush(); acct=a.split(',')[0].strip(); item=None; item_val=None; item_got=False; stack=[]; continue
        if a=='<...>':
